    ...
    def render(self, screen, x, y): #desenhar o grid
        ...
        for li in range(self.linhas): #celulas
            for col in range(self.colunas):
                ...
                pygame.draw.rect(screen, celula.cor, rect=((xCell, yCell, celula.largura, celula.altura)))
...
inven = Inventario(900, grid.identificar_cell(0,0).altura, 1, 8, azul, preto)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if inven.selecionar_cell(pos):
                inven.selecionado(inven.selecionar_cell(pos))
